...
const uni = [];
for (i = 0; i < graduates.length; i++) {
if (graduates[i].university.includes('Uni')){ //includes() method determines whether one string may be found within another string, returning true or false as appropriate.
  uni.push(graduates[i]);
  }
}
console.log(uni.length + ` have "Uni" included in their name`);
...
zooAnimals.forEach(function (i){
  animalNames.push(i.animal_name + "aka" + i.scientific_name);
});
...
*/
const largerPopulation = zooAnimals.filter((pop) => { //removes the last element from an array and returns that element
